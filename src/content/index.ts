...
export class SinglePageModal {
     ...
     */
    private async openModal(animeData: AnimeData): Promise<void> {
        try {
    ...
    showModal(animeData: AnimeData, status: AnimeStatus): void {
        ...
        actions.forEach((action) => {
            if (action.type === "episodeControls") {
                ...
                const episodeInput = episodeControlsContainer.querySelector(
                    ".modal-episode-current",
                ) as HTMLInputElement;
                ...
                button.style.cssText = `
                    ...
                    font-size: 14px;
                    font-weight: 500;
                    ${this.getButtonStyles(action.style)}
                `;

                button.addEventListener("click", () => {
                    this.handleAction(action.type, animeData);
                });
    ...
    private async handleAction(actionType: string, animeData: AnimeData): Promise<void> {
        try {
            switch (actionType) {
                case "addToPlan":
...
// Initialize single page modal for watch pages
const singlePageModal = new SinglePageModal();
